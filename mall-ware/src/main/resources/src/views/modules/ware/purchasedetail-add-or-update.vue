<template>
  <el-dialog
    ...
    <el-form-item label="状态【0->新建；1->已分配；2->正在采购；3->已完成；4->采购失败】" prop="status">
      <el-input v-model="dataForm.status" placeholder="状态【0->新建；1->已分配；2->正在采购；3->已完成；4->采购失败】"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          status: [
            { required: true, message: '状态【0->新建；1->已分配；2->正在采购；3->已完成；4->采购失败】不能为空', trigger: 'blur' }
          ]
        }
      }
    },
